...
> 
> 3. It prints a greeting that includes what `hostname` returns, waits 20 seconds doing nothing, then prints a second message.
> 4. You should expect a **different** hostname than the login node's. Slurm will assign this job to whichever compute node is available, not the login node you're typing commands into.
> {: .source}
{: .solution}
...
>
> Submit the job `broken.sbatch` script.
>
